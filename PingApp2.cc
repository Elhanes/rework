...
Define_Module(PingApp2);

simsignal_t PingApp2::rttSignal = registerSignal("rtt");
simsignal_t PingApp2::numLostSignal = registerSignal("numLost");
simsignal_t PingApp2::numOutOfOrderArrivalsSignal = registerSignal("numOutOfOrderArrivals");
simsignal_t PingApp2::pingTxSeqSignal = registerSignal("pingTxSeq");
simsignal_t PingApp2::pingRxSeqSignal = registerSignal("pingRxSeq");
...
void PingApp2::initialize(int stage)
{
...
void PingApp2::parseDestAddressesPar()
{
...
void PingApp2::handleMessage(cMessage *msg)
{
    EV<<"this is 4";
    if (!isNodeUp()) {
...
void PingApp2::refreshDisplay() const
{
    char buf[40];
...
bool PingApp2::handleOperationStage(LifecycleOperation *operation, int stage, IDoneCallback *doneCallback)
{
    Enter_Method_Silent();
...
void PingApp2::stopSendingPingRequests()
{
    pid = -1;
    lastStart = -1;
    sendSeqNo = expectedReplySeqNo = 0;
...
void PingApp2::scheduleNextPingRequest(simtime_t previous, bool withSleep)
{
    simtime_t next;
...
void PingApp2::cancelNextPingRequest()
{
    cancelEvent(timer);
}

bool PingApp2::isNodeUp()
{
    return !nodeStatus || nodeStatus->getState() == NodeStatus::UP;
}

bool PingApp2::isEnabled()
{
...
void PingApp2::countPingResponse(int bytes, long seqNo, simtime_t rtt)
{
...
std::vector<L3Address> PingApp2::getAllAddresses()
{
 ...
 std::pair<std::string,double> PingApp2:: getpinginfo() // elhanes : new function compare with PingApp.cc
{
     ...
     //pinginfo=std::make_pair(getFullPath(),rttStat.getMin()*1000.0);
